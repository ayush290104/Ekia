...
class _DeliveryManRegistrationScreenState
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'Dispatch Person Registration'.tr),
  ...
  void _addDeliveryMan(AuthController authController) async {
    ...
    if (_fName.isEmpty) {
      showCustomSnackBar('enter dispatch person firstname'.tr);
    } else if (_lName.isEmpty) {
      showCustomSnackBar('enter dispatch person lastname'.tr);
    } else if (_email.isEmpty) {
      showCustomSnackBar('enter dispatch person email address'.tr);
    } else if (!GetUtils.isEmail(_email)) {
      showCustomSnackBar('enter_a_valid_email_address'.tr);
    } else if (_phone.isEmpty) {
      showCustomSnackBar('enter dispatch person phonenumber'.tr);
    } else if (!_isValid) {
      showCustomSnackBar('enter_a_valid_phone_number'.tr);
    } else if (_password.isEmpty) {
      showCustomSnackBar('enter password for dispatch person'.tr);
    } else if (_password.length < 6) {
      showCustomSnackBar('password_should_be'.tr);
    } else if (_identityNumber.isEmpty) {
      showCustomSnackBar('enter dispatch person identitification number'.tr);
    } else if (authController.pickedImage == null) {
      showCustomSnackBar('upload dispatch person image'.tr);
    } else if (authController.vehicleIndex - 1 == -1) {
      showCustomSnackBar('please select vehicle for the dispatch person'.tr);
    } else {
